...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.visionType.findUnique({ where: { id: params.id } });
  if (!existing) return notFoundResponse("Vision type not found");
  const updated = await prisma.visionType.update({ where: { id: params.id }, data: parsed.data });
